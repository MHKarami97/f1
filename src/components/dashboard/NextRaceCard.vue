...
const { nextMeeting, nextRaceSession } = storeToRefs(store)
...
<template>
  <div v-if="nextMeeting" class="card relative overflow-hidden p-6">
    ...
    <h3 class="text-gray-400 dark:text-gray-500 text-xs font-semibold uppercase tracking-widest mb-4">
      مسابقه بعدی
    </h3>
    ...
    <p v-if="nextRaceSession" class="text-f1-red text-sm font-semibold tabular-nums">
      {{ formatDate(nextRaceSession.date_start) }}
    </p>
  </div>
</template>